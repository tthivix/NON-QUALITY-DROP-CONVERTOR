...
const progressStages = [
    ...
    { pct: 65, label: 'Enhancing Quality & FPS...' },
...
function startProgress() {
    ...
    progressLabel.textContent = 'Uploading to AI Cloud...';
...
convertBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        let msg = 'Server busy or API error. Please try again.';
        if (error.message.includes('Cannot reach') || error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
            msg = 'Cannot reach Hugging Face Space. The Space may be sleeping — wait 10s and retry.';
        } else if (error.message.includes('404')) {
            msg = `Endpoint "${ENDPOINT_NAME}" not found. Check your Space API.`;
        } else if (error.message.includes('422')) {
            msg = 'Unsupported video format or backend error.';
...
downloadBtn.addEventListener('click', async () => {
    ...
    const filename  = `${baseName}_60fps_enhanced.mp4`;
